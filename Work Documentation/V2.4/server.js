...
// ─── Profile store ────────────────────────────────────────────────────────────
// Keyed by real client IP, resolved server-side.
// { ip → { displayName, avatar } }
...
// Normalise any IP string the same way getClientIp does
...
app.get('/api/profiles', (req, res) => {
  const out = {};
  for (const [ip, p] of profiles) out[ip] = p;
  res.json(out);
});
...
app.get('/api/profile', (req, res) => {
  ...
  const { displayName, avatar } = req.query;
  const name = typeof displayName === 'string' ? displayName.slice(0, 32).trim() : null;
  const av   = typeof avatar      === 'string' ? [...avatar].slice(0, 2).join('') : null;

  const existing = profiles.get(ip) || {};
  profiles.set(ip, {
    displayName: name ?? existing.displayName ?? null,
    avatar:      av   ?? existing.avatar      ?? null,
  });

  res.json({ ok: true, ip, profile: profiles.get(ip) });
});
...
app.get('/api/whoami', (req, res) => {
  ...
  res.json({ ip, mac: lease.mac || null, hostname: lease.hostname || null });
});
...
app.get('/api/leaderboard', async (req, res) => {
  try {
    ...
    for (const ip of allIps) {
      ...
      const lease   = leaseMap[normIp] || leaseMap[ip] || {};
      const profile = profiles.get(normIp) || profiles.get(ip) || {};
      ...
      entries.push({
        ip: normIp, mac: lease.mac || null,
        hostname: profile.displayName || lease.hostname || nameFromApi[ip] || null,
